...
# Compute repo root and resource paths
...
def collect_by_subclass(g, parent):
    out = []
...
def best_matches(text, vocab, top_k=5):
    t = text.lower()
    scores = []
    for v in vocab:
        vlow = v.lower()
...
def recommend_exams(text, exams):
    desired = []
...
def candidate_conditions(text, conditions):
    seeds = []
...
def run_symptom_triage(symptom_text: str) -> Dict[str, Any]:
    g = load_graph(ONTO_PATH)
    symptoms, exams, conditions = build_catalog(g)
    matched_symptom_names = best_matches(symptom_text, list(symptoms.keys()), top_k=8)
    matched_symptoms = []
