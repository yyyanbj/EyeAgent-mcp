...
async def orchestrator_plan_adapter(state: Dict[str, Any]) -> Dict[str, Any] | None:
    ...
    # derive candidate diseases from screening_results if present
    candidates: List[str] = []
    ...
    # fallback: from previous IA outputs
    if not candidates:
...
async def image_analysis_adapter(state: Dict[str, Any]) -> Dict[str, Any] | None:
    ...
    # populate candidate_diseases if missing
...
async def specialist_adapter(state: Dict[str, Any]) -> Dict[str, Any] | None:
    ...
    # Flatten disease_grades into context for follow-up
...
async def orchestrator_summarize_adapter(state: Dict[str, Any]) -> Dict[str, Any] | None:
    """Let Orchestrator produce the final summary (not simple string concatenation)."""
    rt = _ensure_runtime(state)
    ctx = state.get("context", {})
    ...
    # Use Orchestrator's own reasoning generator
