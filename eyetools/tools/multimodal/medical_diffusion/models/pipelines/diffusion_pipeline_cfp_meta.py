...
class ChamLoss(nn.Module):
    def __init__(self, num_vertex=1448):
        ...
        self.left_mu = nn.Parameter(torch.tensor(left_mu, dtype=torch.float32), requires_grad=False)
        self.left_delta = nn.Parameter(
            torch.tensor(left_delta, dtype=torch.float32), requires_grad=False
        )
        self.right_mu = nn.Parameter(
            torch.tensor(right_mu, dtype=torch.float32), requires_grad=False
        )
        self.right_delta = nn.Parameter(
            torch.tensor(right_delta, dtype=torch.float32), requires_grad=False
        )
    ...
    def forward(self, inputs, targets, eye_labels):
        ...
        dist_1 = torch.mean(torch.min(dist_matrix, dim=-1)[0])
        dist_2 = torch.mean(torch.min(dist_matrix, dim=-2)[0])

        return dist_1 + dist_2

        # # overall
        # loss_1 = self.c_dist(inputs, targets)
        ...
        #     loss_2 += self.c_dist(inputs[b:b+1, mask[b]], targets[b:b+1, mask[b]])
        # loss_2 = loss_2 / batch_size

        # return 0.4 * loss_1 + 0.6 * loss_2

    def c_dist(self, inputs, targets):
        dist_matrix = torch.cdist(inputs, targets, p=2)
        dist_1 = torch.mean(torch.min(dist_matrix, dim=-1)[0])
        dist_2 = torch.mean(torch.min(dist_matrix, dim=-2)[0])
...
class DiffusionPipeline_CFP_META(BasicModel):
    def __init__(
        self,
        noise_scheduler,
        noise_estimator,
    ...
    def _step(self, batch: dict, batch_idx: int, state: str, step: int, optimizer_idx: int):
        ...
        condition0 = batch.get("fundus")
        condition1 = batch.get("eye")
        condition2 = batch.get("age")
        condition3 = batch.get("gender")
        condition4 = batch.get("sph")
        condition5 = batch.get("al")
        condition = [condition0, condition1, condition2, condition3, condition4, condition5]

        # Embed into latent space or normalize
        if self.latent_embedder is not None:
            self.latent_embedder.eval()
            with torch.no_grad():
                print("# VAE Input: ", x_0.shape)
                x_0 = self.latent_embedder.encode(x_0)
        # print("# U-NET Input: ", x_0.shape)
        ...
        mask_seed = torch.rand(1)
        if mask_seed < self.classifier_free_guidance_dropout:
            condition = None
        if (mask_seed >= self.classifier_free_guidance_dropout) & (mask_seed < 0.5):
            condition = [condition0, None, None, None, None, None]
        ...
        loss += self.chamfer_loss(pred, target, condition1)
        ...
        if self.estimate_variance:
            ...
            nnl_loss = torch.mean(
                F.gaussian_nll_loss(pred_x_0, x_0, torch.exp(pred_logvar), reduction="none"),
                dim=list(range(1, x_0.ndim)),
            )
        ...
        with torch.no_grad():
            ...
            results["CD"] = self.chamfer_loss(pred, target, condition1)
        ...
        if self.global_step != 0 and self.global_step % self.sample_every_n_steps == 0:
            ...
            if condition is not None:
                if condition[1] is not None:
                    sample_cond = [c[0 : self.num_samples] for c in condition]
                else:
                    sample_cond = [condition[0][0 : self.num_samples]] + [
                        None for _ in condition[1:]
                    ]

            # sample_cond = [c[0:self.num_samples] for c in condition] if (condition is not None) or (condition[1] is not None) else None
            sample_img = self.sample(
                num_samples=self.num_samples, img_size=x_0.shape[1:], condition=sample_cond
            ).detach()
            ...
            if images.shape[1] == 8:
                ...
                images = images.permute(0, 2, 1, 3)[:, 0:3]
            images = norm(images)
    ...
    def denoise(self, x_t, steps=None, condition=None, use_ddim=True, **kwargs):
        ...
        if use_ddim:
            ...
            timesteps_array = torch.linspace(
                0, self.noise_scheduler.T - 1, steps, dtype=torch.long, device=x_t.device
