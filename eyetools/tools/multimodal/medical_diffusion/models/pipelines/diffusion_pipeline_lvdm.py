...
def video_to_images(x_video):
    """
    assume x.shape: b, c, d, h, w
    return: x_cubes: (bxd, c, h, w)
    """
    batch_size, num_frame = x_video.shape[0], x_video.shape[2]
    x_images = rearrange(x_video, "b c d h w -> (b d) c h w")

    return x_images, num_frame


def images_to_video(x_images, num_frame):
    x_video = rearrange(x_images, "(b d) c h w -> b c d h w", d=num_frame)
...
def ssim_loss(pred, target):
    ...
    """
    loss_sum = 0
    for d_idx in range(pred.shape[2]):
        d_pred = pred[:, :, d_idx]
        d_target = target[:, :, d_idx]
        d_loss = 1 - ssim(
            ((d_pred + 1) / 2).clamp(0, 1),
            (d_target + 1) / 2,
            data_range=1,
            size_average=True,
...
class LVDiffusionPipeline(BasicModel):
    def __init__(
        self,
        noise_scheduler,
        noise_estimator,
    ...
    def _step(self, batch: dict, batch_idx: int, state: str, step: int, optimizer_idx: int):
        results = {}
        x_0 = batch["source"]
        # condition = batch.get('cfp', None)
        condition = batch["cfp"]

        # Embed into latent space or normalize
        # x_0: [B, 1, num_frame, img_size, img_size]
        if self.latent_embedder is not None:
            self.latent_embedder.eval()
            with torch.no_grad():
                x_0, num_frame = video_to_images(x_0)
                # print("input_to_VAE: ", x_0.shape)
                x_0 = self.latent_embedder.encode(x_0)
                x_0 = images_to_video(x_0, num_frame)
                # print("input_to_diffusion: ", x_0.shape)
        ...
        for i, pred_i in enumerate(pred_vertical):
            ...
            # loss += ssim_loss(pred_i, target_i)
        results["loss"] = loss
        ...
        with torch.no_grad():
            ...
            # results['SSIM_loss'] = ssim_loss(pred, target)
        ...
        if self.global_step != 0 and self.global_step % self.sample_every_n_steps == 0:
            ...
            # print("sample: ", x_0.shape[1:])
            # if condition is not None:
            #     print("condition: ", sample_cond.shape)
    ...
    def denoise(self, x_t, steps=None, condition=None, use_ddim=True, **kwargs):
        ...
        if self.latent_embedder is not None:
            x_t, num_frame = video_to_images(x_t)
            x_t = self.latent_embedder.decode(x_t)
            x_t = images_to_video(x_t, num_frame)
            # print("final_output: ", x_t.shape)
