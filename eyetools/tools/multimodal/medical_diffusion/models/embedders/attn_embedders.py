...
def summarize_qkv_chunk(q, k, v, mask, attn_bias_chunk, causal, qk_start_indices, dropout):
    ...
    if causal and q_start_index < (k_start_index + k_chunk_size - 1):
        causal_mask = torch.ones(
            (q_chunk_size, k_chunk_size), dtype=torch.bool, device=device
...
def memory_efficient_attention(
    ...
    for q_index, q_chunk in enumerate(q_chunks):
        ...
        for k_index, (k_chunk, v_chunk, mask_chunk) in enumerate(
            zip(k_chunks, v_chunks, mask_chunks)
        ):
...
class MeshFusionEmbedder_CFP_META(nn.Module):
    def __init__(self, emb_dim=32, num_classes=2, act_name=("SWISH", {})):
        ...
        # age
        self.embedding2 = MLP(
            input_dim=1, hidden_dim=int(emb_dim / 16), output_dim=emb_dim, num_layers=2
        )
        ...
        self.cross_attn = Attention(
            ...
            heads=16,
            memory_efficient=False,
            q_bucket_size=emb_dim,
            k_bucket_size=emb_dim,
        )
        self.norm = nn.LayerNorm(emb_dim)

    def forward(self, condition):
        c_fundus = condition[0].unsqueeze(1)

        c_eye = self.embedding1(condition[1])
        c_age = self.embedding2(condition[2])
        c_gender = self.embedding3(condition[3])
        c_sph = self.embedding4(condition[4])
        c_al = self.embedding5(condition[5])

        c_context = torch.stack([c_eye, c_age, c_gender, c_sph, c_al], dim=1)

        out = self.cross_attn(c_fundus, context=c_context)
        out = self.norm(out)
...
#     context = torch.randn(2, 5, 1024).cuda()
#     # mask = torch.ones(2, 512).bool().cuda()
