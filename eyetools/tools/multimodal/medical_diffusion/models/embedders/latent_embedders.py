...
class VQVAE(BasicModel):
    def __init__(
        ...
        self.quantizer = VectorQuantizer(
            num_embeddings=num_embeddings, emb_channels=emb_channels, beta=beta
        )
...
class VQGAN(VeryBasicModel):
    ...
    def _step(self, batch: dict, batch_idx: int, state: str, step: int, optimizer_idx: int):
        ...
        if optimizer_idx == 0:
            ...
            img_loss = (
                self.vae_img_loss(pred, target, self.vqvae.outc.conv, step, self.discriminator, 0)
                * weights[0]
            )
            ...
            schedulers.append(
                {
                    "scheduler": self.lr_scheduler_vqvae(
                        opt_vqvae, **self.lr_scheduler_vqvae_kwargs
                    ),
                    "interval": "step",
                    "frequency": 1,
                }
