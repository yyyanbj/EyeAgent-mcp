...
from medical_diffusion.models.utils.conv_blocks import EfficientUnetResBlock
...
class Efficient_UNet(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        use_attention = (
            use_attention if isinstance(use_attention, list) else [use_attention] * len(strides)
        )
        # use_attention_middle = ["linear"]*len(strides)  # 1
        ...
        # ConvBlock = EfficientUnetResBlock

        # ----------- In-Convolution ------------
        in_ch = in_ch * 2 if self.use_self_conditioning else in_ch
        self.in_conv = BasicBlock(
            spatial_dims, in_ch, hid_chs[0], kernel_size=kernel_sizes[0], stride=strides[0]
        )
        ...
        middle_blocks = []
        for i in range(4):
            one_middle_block = SequentialEmb(
                EfficientUnetResBlock(
                    spatial_dims=spatial_dims,
            ...
            middle_blocks.append(one_middle_block)
        ...
        self.outc_ver = nn.ModuleList(
            [
                zero_module(
                    UnetOutBlock(spatial_dims, hid_chs[i] + hid_chs[i - 1], out_ch, dropout=None)
                )
    ...
    def forward(self, x_t, t=None, condition=None, self_cond=None):
        ...
        if cond_emb == None:
            emb = time_emb.unsqueeze(-1)
        else:
            emb = torch.stack([time_emb] + cond_emb, dim=2)  # 3
        # print("# emb: ", emb.shape)
        ...
        for i in range(len(self.in_blocks)):
            ...
            x.append(self.in_blocks[i](x[i], None))  # 4

        # ---------- Middle --------------
        h = x[-1]
        for i in range(len(self.middle_blocks)):
            h = self.middle_blocks[i](h, emb)
        ...
        for i in range(len(self.out_blocks), 0, -1):
            ...
            h = self.out_blocks[i - 1](h, None)  # 5
...
if __name__ == "__main__":
    model = Efficient_UNet(in_ch=3, use_res_block=False, learnable_interpolation=False)
