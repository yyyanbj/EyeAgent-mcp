...
class GaussianNoiseScheduler(BasicNoiseScheduler):
    def __init__(
        ...
        if betas is not None:
            ...
            betas = (
                torch.linspace(beta_start**0.5, beta_end**0.5, timesteps, dtype=torch.float64) ** 2
            )
    ...
    def estimate_x_t_prior_from_x_T(
        self, x_t, t, x_T, use_log=True, clip_x0=True, var_scale=0, cold_diffusion=False
    ):
        x_0 = self.estimate_x_0(x_t, x_T, t, clip_x0)
        return self.estimate_x_t_prior_from_x_0(
            x_t, t, x_0, use_log, clip_x0, var_scale, cold_diffusion
        )

    def estimate_x_t_prior_from_x_0(
        self, x_t, t, x_0, use_log=True, clip_x0=True, var_scale=0, cold_diffusion=False
    ):
    ...
    def estimate_x_T(self, x_t, x_0, t, clip_x0=True):
        ndim = x_t.ndim
        x_0 = self._clip_x_0(x_0) if clip_x0 else x_0
        return (self.extract(self.sqrt_recip_alphas_cumprod, t, ndim) * x_t - x_0) / self.extract(
            self.sqrt_recipm1_alphas_cumprod, t, ndim
        )
