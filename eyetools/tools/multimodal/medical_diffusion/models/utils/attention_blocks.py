...
def compute_attention(q, k, v, num_heads, scale):
    q, k, v = map(
        lambda t: rearrange(t, "b (h d) n -> (b h) d n", h=num_heads), (q, k, v)
...
class BasicTransformerBlock(nn.Module):
    def __init__(
        ...
        self.self_atn = LinearTransformer(
            spatial_dims, in_channels, in_channels, num_heads, ch_per_head, norm_name, dropout, None
        )
