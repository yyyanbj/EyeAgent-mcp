    ...
    def __init__(
        ...
        if learnable_interpolation:
            ...
            self.down_op = Pooling(
                kernel_size=kernel_size, stride=stride, padding=get_padding(kernel_size, stride)
            )
...
class EfficientUnetResBlock(nn.Module):
    """
    A modified version of monai.networks.blocks.UnetResBlock with additional skip connection and embedding
...
class UpBlock(nn.Module):
    def __init__(
        ...
        skip_out_channels = (
            out_channels if learnable_interpolation and enable_up else in_channels + skip_channels
        )
        self.learnable_interpolation = learnable_interpolation
